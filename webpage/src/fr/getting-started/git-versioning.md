...
La configuration se fait dans _Contrôle de version git_, dans _Général_, dans les préférences.

![git-note-folder-settings](/img/git/settings.png)

Il faut que git soit installé sur votre système, et le chemin vers l’exécutable git doit soit être dans la variable d’environnement PATH, soit peut être spécifié dans les préférences, dans la section _Contrôle de version git_.
